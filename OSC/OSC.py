...
class OSCClient(object):
    ...
    def __init__(self, server=None):
        ...
        if server == None:
            ...
            self.socket.setsockopt(
                socket.SOL_SOCKET, socket.SO_SNDBUF, self.sndbuf_size)
    ...
    def setServer(self, server):
        ...
        if not isinstance(server, OSCServer):
            raise ValueError(
                "'server' argument is not a valid OSCServer object")
    ...
    def sendto(self, msg, address, timeout=None):
        ...
        try:
            ...
            if e[0] in (7, 65):  # 7 = 'no address associated with nodename',  65 = 'no route to host'
                ...
                raise OSCClientError("while sending to %s: %s" %
                                     (str(address), str(e)))
...
def getRegEx(pattern):
    ...
    pattern = pattern.replace("*", r".*")
    # change '?' to '.' and '{,}' to '(|)'
    pattern = pattern.translate(OSCtrans)
...
class OSCMultiClient(OSCClient):
    ...
    def getOSCTargetStrings(self):
        ...
        for (addr, (prefix, filters)) in self.targets.items():
            out.append(
                ("osc://%s" % getUrlStr(addr, prefix), getFilterStr(filters)))
...
class OSCRequestHandler(DatagramRequestHandler):
    ...
    def dispatchMessage(self, pattern, tags, data):
        ...
        for addr in self.server.callbacks.keys():
            match = expr.match(addr)
            if match and (match.end() == len(addr)):
                reply = self.server.callbacks[addr](
                    pattern, tags, data, self.client_address)
                matched += 1
        ...
        if matched == 0:
            if 'default' in self.server.callbacks:
                reply = self.server.callbacks['default'](
                    pattern, tags, data, self.client_address)
                if isinstance(reply, OSCMessage):
                    replies.append(reply)
                elif reply != None:
    ...
    def _unbundle(self, decoded):
        ...
        if decoded[0] != "#bundle":
            self.replies += self.dispatchMessage(
                decoded[0], decoded[1][1:], decoded[2:])
            return
    ...
    def finish(self):
        ...
        if self.server.return_port:
            self.client_address = (
                self.client_address[0], self.server.return_port)
...
class OSCServer(UDPServer):
    ...
    def setClient(self, client):
        ...
        if not isinstance(client, OSCClient):
            raise ValueError(
                "'client' argument is not a valid OSCClient object")

        if client.server != None:
            raise OSCServerError(
                "Provided OSCClient already has an OSCServer-instance: %s" % str(client.server))
        ...
        client.socket.setsockopt(
            socket.SOL_SOCKET, socket.SO_SNDBUF, client.sndbuf_size)
    ...
    def addDefaultHandlers(self, prefix="", info_prefix="/info", error_prefix="/error"):
        ...
        *sent from* this server. This is ok, because error-messages generally do not elicit a reply from the receiver.
        ...
        if isinstance(self.client, OSCMultiClient):
            self.addMsgHandler(prefix + '/subscribe',
                               self.subscription_handler)
            self.addMsgHandler(prefix + '/unsubscribe',
                               self.subscription_handler)
    ...
    def handle_error(self, request, client_address):
        ...
        self.printErr("%s on request from %s: %s" %
        ...
        if len(self.error_prefix):
            self.sendOSCerror("%s: %s" %
                              (e_type.__name__, str(e)), client_address)
    ...
    def serverInfo_handler(self, addr, tags, data, client_address):
        ...
        if cmd in ('help', 'info'):
            ...
            reply.append(
                ('info_command', "ls | list : list OSC address-space"))
            reply.append(
    ...
    def subscription_handler(self, addr, tags, data, client_address):
        ...
        if len(data):
            if data[0] in ('help', 'info'):
                ...
                reply.append(
                    ('subscribe_command', "[subscribe | listen | sendto | target] <url> [<filter> ...] : subscribe to messages, set filters"))
                reply.append(
                    ('subscribe_command', "[unsubscribe | silence | nosend | deltarget] <url> : unsubscribe from messages"))
                return reply
...
if __name__ == "__main__":
    ...
    op = optparse.OptionParser(
        description="OSC.py OpenSoundControl-for-Python Test Program")
    ...
    op.add_option("-u", "--usage", action="help",
    ...
    if opts.threading:
        s = ThreadingOSCServer(
            listen_address, c, return_port=listen_address[1])
    elif opts.forking:
    ...
    if s.__class__ == OSCServer:
        waitbundle.append(
            "Note how the (single-thread) OSCServer blocks while holding this bundle")
    else:
        waitbundle.append(
            "Note how the %s does not block while holding this bundle" % s.__class__.__name__)
    ...
    if s.__class__ == OSCServer:
        bb.append(
            "Note how the (single-thread) OSCServer handles the bundle's contents in order of appearance")
    else:
        bb.append("Note how the %s handles the sub-bundles in the order dictated by their timestamps" %
                  s.__class__.__name__)
        bb.append(
            "Each bundle's contents, however, are processed in random order (dictated by the kernel's threading)")
